...
const DefaultDatePicker: React.FC<Props> = (props) => {
  ...
  const validateDate = (month: string, day: string, year: string): { isValid: boolean; error: string } => {
    ...
    if (!isValidDate) {
      return { isValid: false, error: "Invalid date (e.g., Feb 31 doesn't exist)" };
    }
      ...
      if (hyphenCount === 2 && inputValue.length >= 8) {
        ...
        if (parts.length === 3) {
          ...
          if (year && year.length < 4 && inputValue.length >= 8) {
            ...
            if (inputRef.current) {
              inputRef.current.setCustomValidity("Year must be 4 digits (e.g., 2024)");
            }
            return;
          }
          ...
          if (month && month.length < 2) {
            ...
            if (inputRef.current) {
              inputRef.current.setCustomValidity("Month must be 2 digits (e.g., 01-12)");
            }
            return;
          }
          ...
          if (day && day.length < 2) {
            ...
            if (inputRef.current) {
              inputRef.current.setCustomValidity("Day must be 2 digits (e.g., 01-31)");
            }
            return;
          }
        }
        ...
        if (inputRef.current) {
          inputRef.current.setCustomValidity("Invalid date format. Use mm-dd-yyyy");
        }
      } else {
    ...
    <div className="relative">
      ...
      <input
        ...
        className={`w-full font-normal bg-background px-3 py-2 rounded-md text-sm shadow-sm border outline-none pr-10 ${
          error ? "border-red-500 focus:border-red-500" : "border-transparent focus:border-gray-300"
        }`}
        type="text"
