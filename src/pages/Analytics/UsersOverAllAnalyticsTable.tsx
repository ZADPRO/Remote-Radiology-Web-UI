...
type Props = {
  analyticsData: OverAllAnalytics[];
};

const OverAllAnalyticsTable: React.FC<Props> = ({ analyticsData }) => {
  // Extract unique users for filter
  const uniqueUsers = useMemo(() => {
    const users = new Set<string>();
    analyticsData.forEach((row) => users.add(row.refUserCustId));
    return Array.from(users).sort();
  }, [analyticsData]);

  const columns: ColumnDef<OverAllAnalytics>[] = [
    {
