...
export function SFormGeneration(
  ...
  const getSurgeryText = () => {
    ...
    if (previousSurgeryYesNo === "Yes") {
      ...
      if (mastectomy === "true") {
        surgeries.push(
          `mastectomy${mastectomyDate ? ` on ${mastectomyDate}` : ""}`
        );
      }

      if (lumpectomy === "true") {
        surgeries.push(
          `lumpectomy${lumpectomyDate ? ` on ${lumpectomyDate}` : ""}`
        );
      }
      ...
      if (cystAspiration === "true") {
        surgeries.push(
          `cyst aspiration${
            cystAspirationDate ? ` on ${cystAspirationDate}` : ""
          }`
        );
      }
      ...
      if (breastReconstruction === "true") {
        surgeries.push(
          `breast reconstruction${
            breastReconstructionDate ? ` on ${breastReconstructionDate}` : ""
          }`
        );
      }
      ...
      if (augmentation === "true") {
        surgeries.push(
          `augmentation${augmentationDate ? ` on ${augmentationDate}` : ""}`
        );
      }
      ...
      if (breastSurgeryOthers === "true" && breastSurgeryOthersSpecify) {
        surgeries.push(
          `${breastSurgeryOthersSpecify}${
            breastSurgeryOthersDate ? ` on ${breastSurgeryOthersDate}` : ""
          }`
        );
      }
  ...
  const getSymptomText = () => {
    ...
    if (lumpOrThick === "true") {
      ...
      if (lumpLeft?.trim())
        lumpParts.push(`left at ${formatClockLabels(lumpLeft)}`);
      if (lumpRight?.trim())
        lumpParts.push(`right at ${formatClockLabels(lumpRight)}`);
      symptoms.push(
        `Lump or thickening${
          lumpParts.length > 0 ? ` in ${lumpParts.join(" and ")}` : ""
        }`
      );
    }
    ...
    if (breastPain === "true") {
      ...
      if (breastPainLeft?.trim())
        painParts.push(`left at ${formatClockLabels(breastPainLeft)}`);
      if (breastPainRight?.trim())
        painParts.push(`right at ${formatClockLabels(breastPainRight)}`);
      symptoms.push(
        `Pain${painParts.length > 0 ? ` in ${painParts.join(" and ")}` : ""}`
      );
    }
    ...
    if (nipplePain === "true") {
      const sides: string[] = [];
      if (nipplePainLeft === "true")
        sides.push(
          "left " +
            (breastSymptoms.nipplePosition.toLocaleLowerCase() === "inverted"
              ? breastSymptoms.nipplePosition.toLocaleLowerCase()
              : breastSymptoms.nipplePositionDetails.toLocaleLowerCase())
        );
      if (nipplePainRight === "true")
        sides.push(
          "right " +
            (breastSymptoms.nipplePositionRight.toLocaleLowerCase() ===
            "inverted"
              ? breastSymptoms.nipplePositionRight.toLocaleLowerCase()
              : breastSymptoms.nipplePositionRightDetails.toLocaleLowerCase())
        );
      symptoms.push(
        `Nipple retraction${
          sides.length > 0 ? ` in ${sides.join(" and ")}` : ""
        }`
      );
    }
  ...
  reportText += `A ${age} year old ${
    pregnant == "Yes" ? "pregnant / lactating" : ""
  } woman${
    ibisScore ? ` with IBIS Tyrer-Cuzic risk score of ${ibisScore}% and ` : ``
  }${auriaResult.toLocaleLowerCase()}${
    mutationSpecify ? `AURIA breast cancer test having ${mutationSpecify}` : ""
  }${hormonRT == "Yes" ? " on hormonal replacement therapy" : ""}${
    previousSurgery.previousSurgeryYesNo === "Yes"
      ? ` who underwent ${getSurgeryText()} surgery`
      : ""
  }.`;
  ...
  if (getPatientAnswer(124) === "Yes") {
    ...
    reportText += ` results are ${getPatientAnswer(126).toLowerCase()}. ${
      getPatientAnswer(127) === "Available"
        ? `The report is ${getPatientAnswer(127).toLowerCase()}.`
        : ""
    }</p>`;
  }
  ...
  if (getPatientAnswer(129) === "Yes") {
    ...
    reportText += ` results are ${getPatientAnswer(131).toLowerCase()}. ${
      getPatientAnswer(132) === "Available"
        ? `The report is ${getPatientAnswer(132).toLowerCase()}.`
        : ""
    }</p>`;
  }
    ...
    if (getPatientAnswer(yesQId) === "Yes") {
      ...
      if (resultsAnswer) {
        if (resultsAnswer.toLowerCase() === "unknown") {
          ...
          text += ` results are ${resultsAnswer.toLowerCase()}`;
        }
      }
  ...
  // Usage
  reportText += appendImagingReport(
