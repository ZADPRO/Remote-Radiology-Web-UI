...
const consentByRole: Record<RoleType, RoleConsent> = {
  ...
  patient: {
    points: [
      "Patients will receive updates and may access their reports securely.",
    ],
    agreeText:
      "I agree to use the platform for receiving my medical imaging results.",
  },
  doctor: {
    points: [
      "I understand that affixing my signature constitutes legal and clinical responsibility for the final report.",
...
const UserConsent: React.FC<UserConsentProps> = ({
  ...
  let userRoleType = role?.type as RoleType;

  if (staticType) {
    userRoleType = staticType as RoleType;
  }
  const userRole = userRoleType ? RoleDisplayNameMap[userRoleType] : "Guest";
  const roleConsent = useMemo(() => {
    return (
      consentByRole[userRoleType] ?? {
        points: [],
        agreeText: "",
      }
    );
