...
import { ListAllPerformingProvider } from "@/services/doctorService";
import { patientService } from "@/services/patientService";
import EditPatient from "./EditPatient";
// import EditScanCenterAdmin from './EditScanCenterAdmin';

const ManagePatient: React.FC = () => {
  const [performingProvider, setPerformingProvider] = useState<
    ListAllPerformingProvider[]
  >([]);
  ...
  const [selectedPerformingProviderId, setSelectedPerformingProviderId] =
  ...
  const getAllPerformingProvider = async () => {
    ...
    try {
      ...
      const res = await patientService.getAllPatient(scanCenterId);
  ...
  const columns = useMemo<ColumnDef<ListAllPerformingProvider>[]>(
    () => [
      {
        accessorFn: (row: ListAllPerformingProvider) =>
  ...
  return (
    <div className="w-full mx-auto">
      ...
      <div className="w-11/12 h-[70vh]  mx-auto my-0 space-y-3 py-4">
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        ...
        >
          ...
          {selectedPerformingProviderId !== null && (
            <EditPatient
              scanCenterId={scanCenterId}
              scanCenterDoctorId={selectedPerformingProviderId}
              setIsEditDialogOpen={setIsEditDialogOpen}
              onUpdate={getAllPerformingProvider}
            />
          )}
        </DialogContent>
      </Dialog>
