...
import {
  doctorService,
  ListSpecificCoDoctor, // Changed from ListSpecificPerformingProvider
} from "@/services/doctorService";
import { Camera, FileText } from "lucide-react";
import React, { useEffect, useRef, useState } from "react";
import { useAuth } from "../Routes/AuthContext"; // Assuming this might be used later or for consistency

const CoReportingDoctorProfile: React.FC = () => {
  const { user } = useAuth();
  // Placeholder values - these would typically come from props, context, or route params
  const scanCenterId = user?.refSCId;
  const coReportingDoctorId = user?.refUserId; // Changed from performingProviderId
  ...
  const getSpecificCoDoctor = async () => {
    ...
    try {
      ...
      if (res.data && res.data.length > 0) {
        ...
        setError("Co-Reporting Doctor data not found."); // Changed text
        setFormData(null);
      }
    } catch (err) {
      console.error("Error fetching co-reporting doctor data:", err); // Changed text
      setError("Failed to fetch co-reporting doctor details."); // Changed text
      setFormData(null);
    } finally {
      setLoading(false);
    }
  ...
  if (!formData) {
    return <div className="p-4">No Co-Reporting Doctor data found.</div>; // Changed text
  }
...
export default CoReportingDoctorProfile;
