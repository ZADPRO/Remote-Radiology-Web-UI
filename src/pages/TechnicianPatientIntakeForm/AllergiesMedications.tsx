...
const AllergiesMedications: React.FC<Props> = ({
    ...
    const renderCheckbox = (label: string, id: number, className: string = "") => (
        <div className={cn(`flex items-center w-30 h-[auto] sm:h-[40px] gap-2 flex-shrink-0 relative`, className)}>
            <Checkbox2
    ...
    return (
        <div className="flex h-full flex-col gap-6 p-4 sm:p-6 overflow-y-auto">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                ...
                <div className="w-full sm:w-2/3">{renderRadioGroup("priority", questionIds.priority, ["Routine", "Urgent"])}</div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row items-start gap-3">
                ...
                <div className="w-full sm:w-2/3 flex flex-wrap gap-2">
                    ...
                    {renderCheckbox("Neoadjuvant Therapy", questionIds.Neoadjuvant, "w-60")}
                    {renderCheckbox("Tamoxifen", questionIds.Tamoxifen, "w-60")}
                    {renderCheckbox("Supplements", questionIds.Supplements, "w-60")}
                    {renderCheckbox("Aromatase Inhibitors", questionIds.Aromatase, "w-60")}
                    ...
                    {getAnswer(questionIds.MedicationOthers) === "true" && (
                        <Input
                            ...
                            onChange={(e) =>
                                handleInputChange(questionIds.MedicationOtherSpecify, e.target.value)
                            }
                        />
                    )}
                </div>
