...
import { ListAllTechnician, technicianService } from '@/services/technicianServices';
import EditTechnician from './EditTechnician';
// 
const ManageTechnician: React.FC = () => {
  const [technicians, setTechnicians] = useState<ListAllTechnician[]>([]);
  ...
  const getAllTechnician = async () => {
    setLoading(true);
    try {
      const res = await technicianService.getAllTechnician(scanCenterId);
      console.log("Fetching technician...", res);
  ...
  const columns = useMemo<ColumnDef<ListAllTechnician>[]>(
    () => [
      {
        accessorFn: (row: ListAllTechnician) => `${row.refUserFirstName} ${row.refUserLastName}`,
        id: 'name',
        header: ({ column }) => (
      ...
      {
        accessorFn: (row: ListAllTechnician) => (row.refUserStatus === "true" ? 'Active' : 'Inactive'),
        id: 'status',
        header: ({ column }) => (
          <div className="flex items-center">
            <span
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <Button
            ...
            onClick={() => {
              setSelectedTechnicianId(row.original.refUserId);
  ...
  const table = useReactTable<ListAllTechnician>({
    data: technicians,
    columns,
    state: {
      sorting,
                  ...
                  {headerGroup.headers.map((header) => (
                    <TableHead
                      ...
                      style={{
                        ...
                        minWidth:
                          (header.column.columnDef as ColumnDef<ListAllTechnician>)
                            .minSize || "auto",
                        maxWidth:
                          (header.column.columnDef as ColumnDef<ListAllTechnician>)
                            .maxSize || "auto",
