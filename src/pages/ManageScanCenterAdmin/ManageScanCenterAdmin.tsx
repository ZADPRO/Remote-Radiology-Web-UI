...
import { ListScanCenterAdmin, scanCenterAdminService } from '@/services/scancenterService';
import EditScanCenterAdmin from './EditScanCenterAdmin';

const ManageScanCenterAdmin: React.FC = () => {
  const [scanCenterAdmin, setScanCenterAdmin] = useState<ListScanCenterAdmin[]>([]);
  ...
  const [selectedCenterAdminId, setSelectedCenterAdminId] = useState<number | null>(null);
  ...
  const getAllScanCenterAdmin = async () => {
    setLoading(true);
    try {
        console.log("Fetching Scan Center Admin...", scanCenterId);
      const res = await scanCenterAdminService.getAllScanCenterAdmin(scanCenterId);
      console.log("Fetching Scan Center Admin...", res);
      setScanCenterAdmin(res.data);
    } catch (error) {
      console.error("Failed to Scan Center Admin:", error);
      setScanCenterAdmin([]);
    }
    finally{
      setLoading(false);
  ...
  useEffect(() => {
    getAllScanCenterAdmin();
  }, []);

  const columns = useMemo<ColumnDef<ListScanCenterAdmin>[]>(
    () => [
      {
        accessorFn: (row: ListScanCenterAdmin) => `${row.refUserFirstName} ${row.refUserLastName}`,
        id: 'name',
        header: ({ column }) => (
          ...
          <div className="flex items-center">
            ...
            {column.getCanFilter() && (
              <Popover>
                ...
                <PopoverContent className="w-48 p-2">
                  <Input
                    placeholder={`Filter Scan Center Admin ID...`}
                    value={(column.getFilterValue() ?? '') as string}
                    onChange={(event: React.ChangeEvent<HTMLInputElement>) =>
                      column.setFilterValue(event.target.value)
                    }
      ...
      {
        accessorFn: (row: ListScanCenterAdmin) => (row.refUserStatus === "true" ? 'Active' : 'Inactive'),
        id: 'status',
        header: ({ column }) => (
          <div className="flex items-center">
            <span
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <Button
            ...
            onClick={() => {
              setSelectedCenterAdminId(row.original.refUserId);
  ...
  const table = useReactTable<ListScanCenterAdmin>({
    data: scanCenterAdmin,
    columns,
    state: {
      sorting,
      globalFilter,
    ...
    <div className="w-full mx-auto">
      ...
      <div className="w-11/12 mx-auto my-0 space-y-3 py-4">
        ...
        >
          ...
          <Button className="bg-[#a4b2a1] hover:bg-[#81927f] w-full lg:w-1/5" onClick={() => navigate("../addScanCenterAdmin", {state: scanCenterId})}>
                    ...
                    <TableHead
                      ...
                      style={{
                        ...
                        minWidth:
                          (
                            header.column
                              .columnDef as ColumnDef<ListScanCenterAdmin>
                          ).minSize || "auto",
                        maxWidth:
                          (
                            header.column
...
export default ManageScanCenterAdmin;