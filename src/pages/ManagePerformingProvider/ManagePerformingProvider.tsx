...
import { doctorService, ListAllPerformingProvider } from '@/services/doctorService';
import EditPerformingProvider from './EditPerformingProvider';
// import EditScanCenterAdmin from './EditScanCenterAdmin';

const ManagePerformingProvider: React.FC = () => {
  const [performingProvider, setPerformingProvider] = useState<ListAllPerformingProvider[]>([]);
  ...
  const [selectedPerformingProviderId, setSelectedPerformingProviderId] = useState<number | null>(null);
  ...
  const getAllPerformingProvider = async () => {
    setLoading(true);
    try {
        console.log("Fetching Performing Provider...", scanCenterId);
      const res = await doctorService.getAllPerformingProvider(scanCenterId);
      console.log("Fetching Performing Provider...", res);
      setPerformingProvider(res.data);
    } catch (error) {
      console.error("Failed to Performing Provider:", error);
      setPerformingProvider([]);
    }
    finally{
      setLoading(false);
    }
  ...
  useEffect(() => {
    getAllPerformingProvider();
  }, []);

  const columns = useMemo<ColumnDef<ListAllPerformingProvider>[]>(
    () => [
      {
        accessorFn: (row: ListAllPerformingProvider) => `${row.refUserFirstName} ${row.refUserLastName}`,
        id: 'name',
        header: ({ column }) => (
          ...
          <div className="flex items-center">
            ...
            {column.getCanFilter() && (
              <Popover>
                ...
                <PopoverContent className="w-48 p-2">
                  <Input
                    placeholder={`Filter Performing Provider ID...`}
                    value={(column.getFilterValue() ?? '') as string}
                    onChange={(event: React.ChangeEvent<HTMLInputElement>) =>
                      column.setFilterValue(event.target.value)
                    }
      ...
      {
        accessorFn: (row: ListAllPerformingProvider) => (row.refUserStatus === "true" ? 'Active' : 'Inactive'),
        id: 'status',
        header: ({ column }) => (
          <div className="flex items-center">
            <span
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <Button
            ...
            onClick={() => {
              setSelectedPerformingProviderId(row.original.refUserId);
  ...
  const table = useReactTable<ListAllPerformingProvider>({
  ...
  return (
    <div className="w-full mx-auto">
                    ...
                    <TableHead
                      ...
                      style={{
                        ...
                        minWidth:
                          (
                            header.column
                              .columnDef as ColumnDef<ListAllPerformingProvider>
                          ).minSize || "auto",
                        maxWidth:
                          (
                            header.column
      ...
      {/* Edit Performing Provider Dialog */}
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent style={{background: "radial-gradient(100.97% 186.01% at 50.94% 50%, #F9F4EC 25.14%, #EED8D6 100%)"}} className="h-11/12 w-[90vw] lg:w-[80vw] overflow-y-auto">
          ...
          {selectedPerformingProviderId !== null && (
            <EditPerformingProvider scanCenterId={scanCenterId} scanCenterDoctorId={selectedPerformingProviderId} setIsEditDialogOpen={setIsEditDialogOpen} onUpdate={getAllPerformingProvider}/>
          )}
        </DialogContent>
      </Dialog>
    </div>
...
export default ManagePerformingProvider;