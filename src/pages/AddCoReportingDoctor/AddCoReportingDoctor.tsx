...
const AddCoReportingDoctor: React.FC = () => {
  const navigate = useNavigate();
  const location = useLocation();
  const scanCenterId = location.state;

  const [formData, setFormData] = useState<NewCoReportingDoctor>({
    firstname: "",
    lastname: "",
    profile_img: "",
    dob: "",
  ...
  const handleFinalSubmit = async () => {
    ...
    try {
      ...
      const res = await doctorService.addCoReportingDoctor(formData);
      console.log(res);
      if (res.status) {
...
export default AddCoReportingDoctor;

interface PersonalDetailsFormProps {
  formData: NewCoReportingDoctor;
  setFormData: React.Dispatch<React.SetStateAction<NewCoReportingDoctor>>;
...
const PersonalDetailsForm: React.FC<PersonalDetailsFormProps> = ({
  ...
  const handleSingleFileUpload = async ({
    ...
    file: File;
    fieldName: keyof NewCoReportingDoctor;
...
const ProfessionalDetailsForm: React.FC<ProfessionalDetailsFormProps> = ({
  ...
  const uploadAndStoreFile = async (
    file: File,
    field: keyof NewCoReportingDoctor,
    tempFileKey: keyof TempFilesState
  ): Promise<void> => {
